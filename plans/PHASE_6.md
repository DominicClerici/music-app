...
| Op | Role | Semantics |
| --- | --- | --- |
| `hat_lift` | drums | the last `hat_closed` event at an offbeat-8th position (`pos % 480 == 240`) in the unit's **second** bar → voice `hat_open`, **dur 360, clamped so the note never sustains past a §3.5 dropout-entered breakdown (S22-10)**, tag `"var"`. No such event → no-op. |
...
*[amended 2026-07-21, S22-10 (user-ratified), C-26: the `hat_lift` duration reads as a **maximum**, not a fixed value. Stage 6 runs `6a HOLD → 6b devices → 6c mutation`; 6b's §3.5 `dropout` truncates every note sustaining across a breakdown entry, and 6c then ran after it — the last offbeat 8th in a bar sits at `pos 1680`, so `+360` extends to 2040, **120 ticks past the bar line**, re-introducing exactly the sustain the dropout had removed and tripping Layer-1 validator **W2**. `hat_lift` is the only operator that lengthens a note, so the clamp is local to this row. The lift is now truncated at the breakdown downbeat, which is what §3.5 already mandates for every other note; §3.5 wins over the literal 360 by user ruling. `fusion_jazz` is the **first pack to combine `hat_lift` with a `breakdown` section** (pop_rock ✓/✗, jazz ✗/✗, chill_lofi ✗/✓, blues ✓/✗), which is why four prior packs never reached it — the fix is a **proven no-op** for all four (185 byte-identical document hashes; no golden or corpus cell moved).]*
