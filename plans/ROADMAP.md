...
| Project tooling & delivery | **Library-first + thin Typer CLI, HTTP deferred** (decided 2026-07-08, pre-implementation): uv for packaging with a committed `uv.lock` and exact-pinned music21/Tone (dependency bumps are deliberate, bless-style events); Ruff (format + lint, with a custom banned-API ruleset enforcing determinism invariant 5) + mypy; pytest + Hypothesis for invariants alongside the parametrized smoke matrix; goldens as committed JSON with a hand-rolled semantic `bless` reporter (no snapshot library — the musical diff is domain-specific); music21 lazy-imported inside the theory wrapper; stdlib `json`. No deploy/host/CI in v1 — the goal is a working algorithm. See `PHASE_1.md` §2/D16. |
...
| Quality & pack-expansion model | **Three-layer validation + IR-level goldens + data-only pack expansion** (decided in Phase 8, 2026-07-07): hard pipeline invariants (W1–W8) + musical rule checks + per-pack statistical bands in committed `calibration.yaml`; a LilyPond-style `bless` workflow over a 60-track golden corpus stored at every IR boundary; pairwise/error-count listening protocol (MUSHRA rejected); audition-CLI-first authoring tooling with coverage linting and selection logs; chill_lofi/blues/fusion_jazz designed with only two additive machinery changes — named humanizer feel profiles with a pack `feelTable` selector, and authored chord-extension tokens (parenthesized, fully pinning); blues ships 4/4 + swing + triplet-grid authoring (no 12/8); lo-fi's loop contrast is `main`/`breakdown` alternation waking the dormant dropout device; all three new packs use patterns-mode bass. See `PHASE_8.md` §3–§9/D1–D16. |
| Golden-value arbitration | **Algorithm text over printed samples** (decided 2026-07-08, pre-implementation): the PHASE docs' worked-example values are derived samples from session reference scripts not kept in the repo; on divergence the pinned algorithm/data text wins, wrong samples are amended alongside the recomputed fixtures, and downstream chained samples are re-verified. See §3 "Golden-value arbitration". |
...
### Golden-value arbitration

Added 2026-07-08, before implementation session 1; binding on every implementation session. The PHASE docs' worked-example values — draw counts, RNG vectors, tick tables, voicings, one seed (`1ps9wxb`) chained PHASE_2 §6.5 → PHASE_3 §7.4 → PHASE_4 §10 → PHASE_5 §9 → PHASE_6 §7 — were computed by session reference scripts that are **not in the repo**. They are derived samples, not sources of truth:

1. **The pinned algorithm and data text is authoritative.** When a faithful implementation diverges from a printed value, first assume an implementation bug. If investigation shows the algorithm text is genuinely ambiguous, resolve the ambiguity explicitly — a PHASE-doc amendment with sign-off, per PROMPT.md's convention — never by tweaking code until it reproduces a printed number.
2. **A wrong printed sample is a doc bug.** Amend it in the same commit that lands the recomputed fixture, and re-verify every downstream phase's chained samples from the first divergent stage onward (a divergence in an early stage invalidates all later docs' printed examples for that seed).
3. **Once committed, fixtures — not doc prose — are the regression surface.** PHASE_8 §8.2's bless discipline (semantic diff, `bless --approve` in a dedicated commit, `generatorVersion` bump) applies in spirit from the moment the first fixture lands, even before the `bless` tooling exists.

PHASE_7 §9 already declares its printed tables illustrative (fixtures assert full-precision recomputation at implementation time); this section generalizes that rule to every phase.
...
- Structural validators: register collisions, fill presence, cadence correctness, density budget compliance — automated checks on generated output (designed as the three-layer suite W1–W8 / L2 / L3, PHASE_8 §8.1).
- Golden-seed regression tests (params + seed → exact expected document) (IR-boundary golden corpus + bless workflow, PHASE_8 §8.2).
- Listening-test workflow for subjective quality (error-spotting + pairwise A/B + anchored rubric, PHASE_8 §8.4).
- Style-pack authoring workflow and full build-out of all five families (Pop/Rock, Chill/Lo-fi, Blues, Jazz, Fusion Jazz) (workflow and tooling PHASE_8 §9; chill_lofi/blues/fusion_jazz designs PHASE_8 §4–§6; reference-pack refinement §7).
