...
- **L2-1 chord-tone-on-strong-beat ratio** (fail below threshold): for bass and comping, the fraction of notes attacking on beats 1/3 whose pitch class ∈ governing chord tones ∪ chord scale *[amended 2026-07-21, S22-13 (user-ratified), C-29: the allowed set is **widened** to `chord tones ∪ chord scale ∪ the alterations PHASE_4 §6.4 already declares legal for the chord's quality`* (for a dominant 7th: 9, ♭9, ♯9, ♯11, 13, ♭13). *Forcing reason: quartal's top voice is a **♯9**, which over a dominant 7th is canonical funk/jazz vocabulary (the Hendrix chord) — L2-1 as pinned was under-modelling altered tensions and failed **19 of 192** fusion renders on musically correct content. The widening is **strictly additive** (it can only admit pitches, never reject one it previously accepted) and reuses §6.4's existing legality table rather than inventing one. **Generation-neutral:** `quality/` is never imported by the generation path, so no golden moves and no `generatorVersion` bump.]*. Thresholds per style live in the pack's calibration artifact (§8.2); engine defaults: bass beat-1 ≥ 0.95, comping strong-beat ≥ 0.98. This is the highest-signal single metric for retargeting/voicing bugs. *[amended 2026-07-21, S23-2 option D (user-ratified), C-32: the allowed set is **widened once more**, to admit the **perfect fourth above the chord root** (the natural 11) — but **only for a `(role, section)` whose pack voicing classes declare `quartal`* (`pack.voicing[role].classes[rung]`, read at the rung the arrangement assigned that section). *Forcing reason, the exact shape of C-29's: `theory/voicing.py:185` resolves `quartal` to `[[0, 5, 10, 15]]`, so offset 5 — the natural 11 — is a **structural voice of every quartal voicing**, and §6.4 pins quartal as fusion_jazz's signature low-rung comping harmony. §6.4's tension table withholds `11` from a dominant 7th, correctly as a rule about *dressing a chord symbol* (the avoid note that makes `7sus4` a separate quality) — but here the fourth is not dressing, it is a voice the pack's own pinned table was told to sound. L2-1 was therefore counting the pack's signature voicing as wrong. **Mechanism, and why it is narrow:** nothing downstream of `parts/voicing.py::build_voicing_map` records which candidate class won the Viterbi pass, so per-note provenance does not exist; the widening instead keys off the pinned pack data that *produced* the candidate. That confines it on three independent axes — only roles with a `voicing:` block (`bass` has none, on any pack), only rungs naming `quartal` (of the five shipped packs, only fusion_jazz comping rungs 1–2; jazz's quartal is on **pads**, which L2-1 does not measure), and only sections at such a rung (fusion's rungs 3–4 are unaffected). **Known over-approximation, stated here so §8.1 read alone does not sound tighter than the mechanism is:** per-note provenance does not exist, so a group is widened whenever its rung **declares** quartal — even for a chord where the Viterbi DP actually chose one of that rung's other classes (`rootless_a`/`rootless_b`) and no quartal fourth ever sounded. The fourth is *admitted*, not *required*; tightening it would need per-note voicing provenance the pipeline does not carry. Measured exposure — **strongly sweep-dependent; do not quote a single figure**: three independent sweeps put fusion_jazz's share of strong-beat measured notes sitting in a widened group at **6.01 %** (orchestrator, 10 seeds × all moods × 5 lengths = **118 123** measured notes — the largest sample), **8.11 %** (T2 review) and **12.74 %** (T9, 3 seeds × 2 lengths = 10 270). The spread is ~2×; treat the exposure as *single-digit-to-low-teens percent* and re-measure rather than cite. The **0 %** on each of the other four packs, by contrast, is **exact, structural and reproduced in all three sweeps** — 0 of 423 340 measured notes across blues, chill_lofi, jazz and pop_rock in the orchestrator sweep alone — because no other shipped pack declares `quartal` on a role L2-1 measures. **That 0 % is the load-bearing claim; the fusion percentage is context.** **Strictly additive**, like C-29: **at most one** pitch class is ever unioned in and none is ever removed, so nothing that passed before can fail now — "at most", not "exactly", because `{root + 5}` is frequently already in the narrow set, making the net addition zero (the test asserts the honest bound, `wide − narrow ⊆ {root + 5}`). **Measured at depth** — 80 seeds × all moods × 5 lengths (60/120/180/240/300 s) × 5 packs, 18 000 renders / 36 000 (track, role) rows: fusion comping fell from **30 failing rows to 0** (451 out-of-set strong-beat notes to 0), the four other packs measured **byte-identically — 0 rows changed, 0 failures before or after**, and fusion bass was untouched. No threshold moved. **Generation-neutral:** `quality/` is never imported by the generation path, so no golden moves and no `generatorVersion` bump.]* *[amended 2026-07-21, S23-1, C-31 — **grain, a binding semantic pin**: "notes attacking on beats 1/3" means attacks on the **stage-6 (pre-humanizer) grid**, so L2-1 measures **`trace.phrases_stage6`**, *not* `doc.tracks`. This is the grain W7 already uses (§3.1 grid homogeneity, `quality/layer1.py`). *Forcing reason: stage 7's swing and jitter displace strong-beat onsets off ticks 0/960 **by design**, so an exact `ticks % 1920 ∈ strong` filter over the humanized document discards most of the population the check is defined over — measured at **5–18 %** retention, and **0 %** for both jazz and chill_lofi comping, where the check silently degenerated to a **vacuous pass** (`total == 0` → skip) in 12 of 12 renders. §8.1's own "highest-signal single metric" was reading none of two packs' comping.* **A role that emits pitched notes but yields no measurable strong-beat note is now reported**, on the non-gating channel with its own `L2-1-SKIP:` prefix, so the vacuous case can never again be mistaken for a clean pass; a role that emits nothing at all stays quiet. **Generation-neutral:** `quality/` is off the generation path — no golden moved, no `generatorVersion` bump. **Any L2-1 figure measured before this change is on the wrong grain and is not comparable with one measured after.**]*
...
- **Smoke matrix**: every pack × supported mood × 3 length buckets (60/180/480 s) × 5 seeds, running Layers 1–2 — the five-pack generalization of every prior phase's property tests. Periodically (nightly/weekly), a 300-seed sweep on the two reference packs bounds per-cell failure below ~1 % (rule of three). *[annotated 2026-07-21, S19-3 + S23-12 (both user-authorized), C-19 + C-33: **two of the five packs cannot fill the 480 s bucket, by design — this is not a defect and must not be "fixed".** Form length is bounded above by each form template's **authored repeat counts**, not by `maxLengthSec` (which is a maximum, not a target), so a pack whose templates are short saturates below the budget. Measured: **`pop_rock` tops out at a 104-bar / **356.57 s** ceiling, uniform across all 11 moods** (C-19), and **`chill_lofi` at 56 bars — 183.53 s at the pinned probe seed, 197.65 s max across 8 moods × 5 seeds** (C-33), flat between the 180 s and 480 s buckets on 36 of 40 cells. `blues`, `fusion_jazz` and `jazz` all reach **480.00 s exactly**. Both ceilings are **accepted** — an 8-minute pop_rock form and an 8-minute lo-fi loop are out of genre, and raising either pack's `forms.yaml` repeat bounds would amend pinned pack data against the pack's own character. Consequence for the tests: `tests/test_smoke_matrix.py`'s per-pack floors in `_CEILING_FLOOR_SEC` (`pop_rock` 340.0, `chill_lofi` 176.0) **deliberately encode these ceilings and must not be raised to 480**, and chill_lofi's long-form growth guard runs against a per-pack *reference* bucket (`_PACK_REFERENCE_SEC = {"chill_lofi": 60}`) rather than being exempted. The bucket list itself is unchanged: every bucket is still rendered and gated for every pack.]*
...
- *[amended 2026-07-21, session 24 (C10), session-22 anomaly A3 closed: the audition CLI also carries **`--ensemble <preset>`** and **`--role-flavors <role=flavor,…>`** / repeatable **`--role-flavor <role=flavor>`**, populating the `ensemblePreset` / `roleFlavors` raw-params (validated downstream). Before this, comparing two timbres in-ensemble forced a `generate --params` fallback (blocking the T6 flavor auditions); the flags let the edit→hear loop reach every flavor/preset directly, and the sibling `trackgen ab` harness (§8.4) reuses the same axis.]*
...
4. **PHASE_4 §3.1**: token grammar gains the parenthesized extension group; authored extensions fully pin the token (dressing skips, draw-free) (§3.5). New loader rule **P11**: authored extensions legal per §6.4 for the token's quality. **§12 Q1**: resolved (§3.5). **§12 Q8**: closed not-needed (§3.8). **§12 Q2** (slash bass): **stays unexercised, Post-v1** — grammar-complete and P-rule-fixtured, but no v1 pack uses it (§3.8). *[wording corrected 2026-07-21, session 23 (C9) T9, DoD §14.11 audit: this line previously read "**§12 Q2/Q8**: closed not-needed", collapsing two different dispositions. PHASE_4 §12's own table annotates **Q2** as still unexercised with `Post-v1` in its resolves column — which agrees with §3.8's authoritative row ("Stay unexercised / not needed", i.e. Q2 unexercised, Q8 not needed). **The target document was right and this summary's word was the loose one**; only §13 is changed. All 8 §13 items were verified APPLIED in the same audit, so §14.11 is MET.]* **§12 Q3**: stays deferred, evidence strengthened (§3.8). **§12 Q7**: deferred post-v1 (§3.8).
...
7. **Tooling**: audition CLI with `--section`/`--solo`/`--mute`/`--play` *[+ `--ensemble`/`--role-flavors` per §9.1's C10 amendment]*; pack linter (errors + all five warning classes) *[the unreachable-content class is per-mood × per-section-kind faithful and silenced per-pattern-id as of C10]*; `--explain` selection log; `trackgen calibrate` producing `calibration.yaml`. *[C10 also adds the two §8.4 listening tools: `trackgen ab` (pairwise A/B harness) and `trackgen rubric` (anchored milestone rubric).]*
