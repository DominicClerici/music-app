...
> **Next:** Phase 4 **Chunk 2** (SESSION_05) — the harmony **stage** + goldens. Chunk 1 (SESSION_04) is **complete**: the theory library (`src/trackgen/theory/chords.py`+`voicing.py`), the dressing ladder (`src/trackgen/harmony/dressing.{py,yaml}`), and the `progressions.yaml` loader + reference packs all built, reviewed, and committed (587 tests, four gates green). DoD 1/2/3/8 PROVEN. Commits: T1 `21ce323`, T2 `6cc5907`, T3 `ee7ddb6`, T4 `bb7114e`, lane-fix/close-out in the docs commit.
>
> **Chunk 2 scope (DoD 4/5/6/7/9/10) — build `harmony(plan, form, progressions, rng) -> HarmonicPlan` (§5.1) in `src/trackgen/harmony/stage.py`:**
>  1. **Schema extension first** (`src/trackgen/schema/ir.py`, §7): add `HarmonicPlan.keys: [{startTick, tonicPc, mode}]` (one region in v1), per-`ChordEvent` `scale: {rootPc, name}` + `function: T|S|D|O` + `tags: [str]`, and `HarmonicPlan.poolSelections: {str: str}`. These are the reserved extension points — additive to the pinned core.
>  2. **The stage** §5.1: tag-selection (one draw per distinct harmonyTag, gated by mode/valence/dissonance, `weighted_choice` iff ≥2), the density soft-filter §5.2 (`harmonicRhythmBase`), per-slot dressing draws (call `harmony.dressing.dressing_options(...)` → draw iff ≥2 options), timeline assembly with hold-merge, then the **three boundary transforms** §5.4/§5.5 (turnaround swap on same-tag boundaries; deceptive fallback — dormant in v1; mandatory final close from `finals`). RNG: `stream_rng(master, overrides, "harmony")`, append-only draw order per §5.6.
>  3. **Goldens/tests:** §10 event-for-event chains (Ex1 76 events / Ex2 64 events), §5.6 seed vectors *in the stage draw sequence*, determinism (counting-RNG shim: **8 draws** Ex1, **30 draws** Ex2; singleton-candidate = 0 draws), DoD-7 property matrix, DoD-9 synthetic deceptive fixture, §13 amendment-consistency check.
>
> **Ready-made building blocks (Chunk 1, already committed):** import from `trackgen.theory` — `resolve_token(token, key)`, `chord_function(token)→"T"/"S"/"D"/"O"`, `chord_scale(spec, key)→ScaleHint(root_pc,name)`, `chord_symbol(spec, key)`, `legal_extensions`/`extensions_legal`; from `trackgen.harmony.dressing` — `tier(diss)`, `effective_tier(base,fn)`, `dressing_options(spec, was_bare, function, base_tier, key)→[(ChordSpec,weight)]` (returns the ordered option list; **the stage performs the draw**). From `trackgen.packs` — `StylePack.progressions: ProgressionsConfig|None` (pools/turnarounds/finals; `PoolEntry.density` cached; `.final_chord_token()`). Loader already enforces P1–P10 + cross-file P1/P4/P6/P7.
>
> **Chunk-2 handoff notes (from the whole-chunk review — read before writing goldens):**
>  - **`symbol` is ASCII, not the §10 prose glyphs.** `min7b5.symbol == "Em7b5"` (§3.3 rule 3), NOT the `Eø7` printed in §10.2; flats are `b` (`A7b13`, not `A7♭13`). Assert the ASCII strings; **do not** tune `chord_symbol` to reproduce the prose. (Consistent with the §10.1 sample-event JSON showing `"B7"`.)
>  - **Secondary dominants are T/S-function by §3.2** (`III7`/`VI7`→T, `II7`/`bII7`→S), so dressing *cools* them (T−1/S+0), not D+1. Correct-by-design; the `function` field does not capture secondary-dominant tension. The Chunk-2 **turnaround transform** must judge "dominant-functioning relaunch" the same way P8 does (see [C-03] `_relaunches_as_dominant`: D-function OR dom7 rooted on pc 1), not by the raw §3.2 label.
>  - **[C-03]** (open, user-approved Option A): P8 admits the SubV `bII7` as dominant-functioning. **[C-04]** (open): `voicing_candidates` is keyless so `quartal`=perfect-4ths `[0,5,10,15]`; `optimal_voicing_path` gained an additive keyword-only `anchor`; `rootless_a/b`/`drop2` degrade on triads. Both deferred to PHASE_5 §13.6 (voicing goldens) — not Chunk 2's concern, but don't re-pin them.
>  - **`poolSelections` keys** (§7.3): each harmonyTag; `"turnaround:<sectionId>"` per swapped boundary; `"finals"`.
>  - **Orchestrator pre-verify (reuse):** the §5.6 harmony seed vectors already reproduce exactly (`derive(3735928559,"harmony")==226146634901021418`; getrandbits `[1607822876,501707672,365345814,982234362,2945966636]`; randrange(100) `[47,14,10,29,87]`). All 10 §10 per-chord facts (symbol/function/eff-tier/scale) independently reproduced by the orchestrator. Before the Chunk-2 gate, reproduce the full 8/30-draw sequences from these packs.
>
> **Post-Phase-3 status (unchanged):** Phase 3 complete; V2 validator hardened; determinism (`from_base36` canonical-only + TID251 bans on `secrets`/`uuid`/extra `datetime`).
> **`SongForm` (Chunk-2 input) recap:** `form(plan, forms) -> SongForm` (`src/trackgen/form/stage.py`) emits `sections[{id, type, index, total_of_type, start_bar, length_bars, energy, phrases:[{label,bars}], harmony_tag, variant, ending}]`, `total_bars`, `template_id`. `harmony_tag` keys the pools; same phrase `label` ⇒ same harmonic material; `ending` (final section only) drives the final close; `SongForm` carries **no cadence field** by design (Phase 4 owns cadence). Cadence tendencies to honor: PHASE_3 §3.2 semantics table.
> **Carry-forward for later phases (unchanged):** Phase 5 Serializer must dump documents with `exclude_none=True`; pack models enforce `extra="forbid"` (new fields added explicitly); env `uv` manages Python 3.12.13, all four gates green.
...
| 4 | Harmony engine | in progress | 04 | Chunk 1 done (SESSION_04: theory+dressing+loader; DoD 1/2/3/8 proven, 587 tests). Chunk 2 remains (SESSION_05: stage+goldens; DoD 4/5/6/7/9/10) |
