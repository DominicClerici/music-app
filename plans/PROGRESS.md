...
> **Phase 1 leftover (non-blocking):** the §9.6 manual listening check still awaits the user — open `playground/index.html` (static server: `uv run python -m http.server` from repo root → `http://localhost:8000/playground/`), audition `fixtures/milestone.trackdoc.json` against the six checklist items. A surfaced defect is a short Phase-1 follow-up.
