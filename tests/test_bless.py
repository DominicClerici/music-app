...
# length. The committed half covers all 60.
...
def test_matching_baseline_is_clean(cell: corpus.Cell, tmp_path: Path) -> None:
    ...
    # One cell of 60 is a scoped run, so the diff report is followed by the
...
# written into every cell's `document.json`. A bump therefore changes all 60
...
# `bless --approve --pack pop_rock` restamps 12 cells and leaves the other 48
...
def test_cli_pack_scoped_run_says_it_was_scoped() -> None:
    ...
    assert "SCOPED RUN — 12 of 60 corpus cell(s) selected" in result.stdout
    assert "the other 48 were NOT re-rendered" in result.stdout
...
def test_committed_corpus_is_complete() -> None:
    """Non-vacuity: 60 cells × 10 stages actually on disk (ROADMAP §3)."""
    assert len(_CELLS) == 60
    assert len({cell_id(c) for c in _CELLS}) == 60
    for cell in _CELLS:
...
def test_bless_on_the_committed_corpus_is_clean() -> None:
    ...
    assert format_result(result) == "bless report — 60 cell(s), no divergence."
