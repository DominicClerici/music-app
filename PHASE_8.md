...
- **Arrangement**: `layersMax` 3 (A ≤ 0.3) → mains run drums+bass+comping; pads enter never at defaults (count = min(3, base 3)) — pads are reserved for higher-arousal moods (happy/dreamy at layersMax 3 with rung-3 sections) or the `study` ensemble's fuller mixes; breakdowns drop to drums+bass, then dropout cuts the entry. densityBudget main-1 = `0.321 × (0.7 + 0.6 × 0.413)` = **0.304** — most `minDensity`-gated ornaments (0.45–0.75) stay out: sparse, as designed.
